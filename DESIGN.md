...
3. Default body to `{typography.body-md}`; use `{typography.code}` for any developer-facing snippet.
4. Keep emerald scarce; one filled green button per viewport.
5. The white-canvas commitment is non-negotiable — adding atmospheric backdrops breaks the brand.